...
def connecting_routes
  ...
  FROM
    routes
  WHERE
  -- What is the difference between pos, num and stop_id?
...
def craiglockhart_to_sighthill
  ...
  JOIN
    routes finish on (finish.company = from_transfer.company AND finish.num = from_transfer.num)
  JOIN
    stops start_stop ON (start.stop_id = start_stop.id)
  JOIN
    stops end_stop ON (finish.stop_id = end_stop.id)
  WHERE
    start_stop.name = 'Craiglockhart' AND end_stop.name = 'Sighthill'
  SQL
end
